...
/**
 * <p>
 * Scattering: 将数据写入到buffer，可以采用buffer数组，一次写入【分段】 Gathering: 从buffer 读取数据时，可以采用buffer数组，一依次读
 * </p>
 *
 * @author chachae
 * @version v1.0
...
public class ScatteringAndGatheringDemo {
  ...
  /**
   * 客户端使用 telnet 客户端发送一条数据，如果数据量小于8，那个一次while循环即可将数据全部读取写进缓存并返回给客户端， 如果数据量大于8，那么在读取完8个数据之后，写进缓存并返回给客户端，然后再次读取数据。
   * <p>
